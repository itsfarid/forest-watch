...
/* =========================
   ...
   - optional: ROBOFLOW_CONFIDENCE_THRESHOLD (0..1) default 0.95
   - optional: ROBOFLOW_OVERLAP_THRESHOLD (IoU) default 0.5
   ========================= */
...
const ROBOFLOW_CONFIDENCE_THRESHOLD = parseFloat(process.env.ROBOFLOW_CONFIDENCE_THRESHOLD ?? '') || 0.95;
const ROBOFLOW_OVERLAP_THRESHOLD = parseFloat(process.env.ROBOFLOW_OVERLAP_THRESHOLD ?? '') || 0.5;
...
function extractPredictionsFromResponse(rfResponse: any): any[] {
  ...
  // Some Roboflow responses may nest predictions; attempt to find any array named predictions
...
/* =========================
   NMS and bounding-box helpers
   - Roboflow boxes typically use center x,y,width,height
   ========================= */
function toBoxXYXY(p: any) {
  const x = p.x ?? 0;
  const y = p.y ?? 0;
  const w = p.width ?? 0;
  const h = p.height ?? 0;
...
function iou(a: any, b: any) {
  ...
  const aArea = Math.max(0, a.xMax - a.xMin) * Math.max(0, a.yMax - a.yMin);
  const bArea = Math.max(0, b.xMax - b.xMin) * Math.max(0, b.yMax - b.yMin);
  const union = aArea + bArea - interArea;
  return union <= 0 ? 0 : interArea / union;
}

function nms(preds: any[], iouThreshold = 0.5) {
  const sorted = preds.slice().sort((a, b) => (b.confidence ?? 0) - (a.confidence ?? 0));
  const keep: any[] = [];
  for (const p of sorted) {
    const boxP = toBoxXYXY(p);
    let skip = false;
    for (const k of keep) {
      const boxK = toBoxXYXY(k);
      if (iou(boxP, boxK) >= iouThreshold) {
        skip = true;
        break;
      }
    }
...
export async function continueConversation(
  ...
  if (last.role === 'user' && typeof last.content === 'string' && last.content.startsWith('data:image')) {
    try {
      ...
      // Robust extraction of raw predictions
      const rawPreds: Array<{ confidence?: number; class?: string; x?: number; y?: number; width?: number; height?: number }> =
        extractPredictionsFromResponse(rfResponse);

      // Apply confidence threshold + NMS to match Roboflow UI behavior
      const confidenceThreshold = ROBOFLOW_CONFIDENCE_THRESHOLD; // e.g. 0.95
      const overlapThreshold = ROBOFLOW_OVERLAP_THRESHOLD; // e.g. 0.5 IoU

      const confFiltered = rawPreds.filter((p) => (p.confidence ?? 0) >= confidenceThreshold);
      const finalDetections = nms(confFiltered, overlapThreshold);

      const total = finalDetections.length;
      const avgConf = total > 0 ? finalDetections.reduce((s, p) => s + (p.confidence ?? 0), 0) / total : 0;

      const top = finalDetections
        .slice()
      ...
      if ((!finalDetections || finalDetections.length === 0) && ROBOFLOW_DEBUG) {
