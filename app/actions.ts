    ...
    if (isImageUrl) {
      try {
        ...
        const detectionCount =
          outputs.find((o) => o.count_objects !== undefined)?.count_objects ||
          0;
        const predictions =
          outputs.find((o) => o.predictions)?.predictions || [];
        const visualizedImage = outputs.find((o) => o.output_image)
          ?.output_image?.value;
        ...
        if (predictions.length > 0) {
          ...
          uniqueClasses.forEach((cls, idx) => {
            ...
            const totalConf = classItems.reduce(
              (sum, p) => sum + p.confidence,
              0
            );
            const avgConf = totalConf / count;
            responseContent += `${idx + 1}. ${cls}: ${count} items (avg ${(
              avgConf * 100
